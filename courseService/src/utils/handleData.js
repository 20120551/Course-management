...
module.exports.GenerateSignature = (payload)=> {
    const {_id, username, listOfPermissions, isActive} = payload;

    const accessToken = jwt.sign({_id, username, listOfPermissions, isActive}, ACCESS_TOKEN, {expiresIn: '30d'});

    const refreshToken = jwt.sign({_id, username, listOfPermissions, isActive}, REFRESH_TOKEN, {expiresIn: '30d'});
